...
### 🔍 핵심 로직 고도화 계획
* **데이터 기반 피드백:** PostgreSQL DB와 연동하여 과거 매매 성과를 학습하고, 손실 발생 패턴을 분석해 AI 추천 로직의 정확도를 향상시킵니다.
* **통합 관리 대시보드:** 텍스트 알림의 휘발성을 보완하기 위해 실시간 자산 변동 차트와 원클릭 제어가 가능한 전용 웹 인터페이스를 제공할 예정입니다.
